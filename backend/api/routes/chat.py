...
async def _handle_chat(request: CopilotRequest, registry: ModelRegistry) -> CopilotResponse:
    llm = LLMClient()
...
async def chat(request: CopilotRequest, registry: ModelRegistry = Depends(get_registry)):
    return await _handle_chat(request, registry)
